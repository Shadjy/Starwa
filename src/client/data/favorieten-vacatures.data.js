(function(){
  ...
  // This page shows only 'vacature' favorites
  ...
  function render(){
    const favs = getFavs();
    const filtered = favs.filter(f => f.type==='vacature');
    if(!filtered.length){
      listEl.innerHTML = `<div class="card"><p>Nog geen favorieten. Ga terug en klik op <em>Favoriet</em> bij een vacature of kandidaat.</p></div>`;
    ...
    listEl.innerHTML = filtered.map(f=>{
      const d = f.data;
      const head = f.type==='vacature'
        ? `<strong>${fmt(d.title)}</strong><div class="meta">${fmt(d.company)} • ${fmt(d.locatie)} • ${fmt(d.uren)}u • ${fmt(d.functie)} • ${fmt(d.sector)} • ${fmt(d.contractType)}</div>`
        : `<strong>${fmt(d.name)}</strong> — ${fmt(d.currentTitle||d.desiredRole)}<div class="meta">${fmt(d.locatie)} • ${fmt(d.uren)}u • ${fmt(d.sector)} • ${fmt(d.contractType||'-')}</div>`;
  ...
  function renderCompare(){
    const favs = getFavs().filter(x=>x.type==='vacature' && compareIds.has(x.id));
  ...
  clearAll.addEventListener('click', ()=>{
    const keep = getFavs().filter(f => f.type !== 'vacature');
    setFavs(keep);
    compareIds.clear();
    render();
  });
