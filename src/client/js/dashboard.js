document.addEventListener("DOMContentLoaded", () => {
  ...
  if (messagesViewAll) {
    if (messagesViewAll.tagName.toLowerCase() === 'a') messagesViewAll.setAttribute('href', '/berichten1')
  ...
  const loadMessages = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/berichten1?limit=3`, { credentials: 'include' })
      if (!res.ok) throw new Error('not_ok')
      const data = await res.json()
