const ICONS = {
  ...
  calendar: `<svg viewBox="0 0 20 20"><path d="M3.5 7h13m-12 9h11a1.5 1.5 0 0 0 1.5-1.5V5.5A1.5 1.5 0 0 0 16.5 4h-13A1.5 1.5 0 0 0 2 5.5v9A1.5 1.5 0 0 0 16.5 16Zm0 0V5a1.5 1.5 0 0 1 1.5-1.5h5a1.5 1.5 0 0 1 1.5 1.5v1.25h2.5a1 1 0 0 1 1 1V15a1 1 0 0 1-1 1h-15a1 1 0 0 1-1-1V8.25a1 1 0 0 1 1-1H5.5V6a1 1 0 0 1 1-1Zm0 0V4a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v1" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"></path></svg>`,
  message: `<svg viewBox="0 0 20 20"><path d="M3.25 4.75h13.5c.55 0 1 .45 1 1v8.5c0 .55-.45 1-1 1h-4.5l-3.25 2.5V15.25H3.25c-.55 0-1-.45-1-1v-8.5c0-.55.45-1 1-1Zm0 0V4a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v1" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linejoin="round"></path></svg>`,
  ...
  trash: `<svg viewBox="0 0 20 20"><path d="M4.5 6.5h11M12 6.5v8m-4 0v-8M7 6.5 7.5 4h5l.5 2.5m1.5 0V16a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1V6.5Zm0 0V4a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v1" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"></path></svg>`,
...
const seedNotifications = [
  {
    ...
    message: "Je bent gematcht met de functie Senior Frontend Developer bij TechCorp.",
    detailMessage:
  ...
  },
  {
    ...
    message: "DesignStudio heeft je een bericht gestuurd over je sollicitatie.",
    detailMessage:
...
const elements = {
  ...
  backToDashboard: document.getElementById("backToDashboard"), // Added for back to dashboard button
}
...
const formatRoleLine = (notification) =>
  notification.role ? `${notification.role} · ${notification.company}` : notification.company
...
const applyFilters = () => {
  ...
  return state.notifications
    .filter((note) => {
      ...
      const haystack = [note.title, note.message, note.company, note.role, TYPE_CONFIG[note.type]?.label]
        .filter(Boolean)
...
const updateMarkAllButton = () => {
  ...
  elements.markAllButton.setAttribute("aria-disabled", hasUnread ? "false" : "true")
}
...
const renderEmptyState = () => {
  ...
  message.textContent = "Pas je filters aan of verwijder de zoekopdracht om opnieuw te proberen."
...
const renderNotifications = () => {
  ...
  list.forEach((note) => {
    ...
    avatar.setAttribute("aria-label", `Bedrijf ${note.company.substring(0, 40)}`)
...
const updateModal = (note) => {
  ...
  elements.modalMessage.textContent = note.detailMessage ?? note.message ?? ""

  elements.modalMarkRead.textContent = note.read ? "Markeer als ongelezen" : "Markeer als gelezen"

  elements.modalSaveText.textContent = note.saved ? "Verwijder uit favorieten" : "Opslaan als favoriet"
}
...
const handleCardInteraction = (event) => {
  ...
  if (target.closest("button") && target.closest("button").dataset.action === "delete") {
...
const initEventListeners = () => {
  ...
  // Added for back to dashboard button handler
...
const init = () => {
  populateTypeFilter()
  initEventListeners()
  loadConversations() // Initialize by loading real data
}
...
async function loadConversations() {
  try {
    ...
    if (data.conversations && data.conversations.length > 0) {
      ...
      const notifications = data.conversations.map((conv, index) => ({
        id: `chat-${conv.id}`,
        type: "message",
        title: `Message from ${conv.contact_name}`,
        message: conv.last_message || "No messages yet",
        detailMessage: conv.last_message || "Start a conversation",
        company: conv.contact_name,
        role: "Chat",
        postedAt: conv.created_at,
        timeAgo: getTimeAgo(new Date(conv.created_at)),
        read: index > 0,
        saved: false,
        badge: index === 0 ? "Nieuw" : "",
        badgeClass: index === 0 ? "badge--info" : "",
        status: "Active",
      }))
...
function getTimeAgo(date) {
  ...
  if (seconds < 86400) return `${Math.floor(seconds / 3600)}h geleden`
