document.addEventListener("DOMContentLoaded", async () => {
  ...
  const renderVacancies = (items = []) => {
    ...
    items.forEach((v) => {
      ...
      el.innerHTML = `
        ...
        ${
          Array.isArray(v.vaardigheden) && v.vaardigheden.length
            ? `<div class="skills">${v.vaardigheden
                .slice(0, 6)
  ...
  const focusFirstCompany = () => {
    const first =
      document.getElementById("cp_bedrijfsnaam") || companyModal?.querySelector("input,select,textarea,button")
    first?.focus?.()
  }
  const updateCompanyFooterShadow = () => {
    if (companyFooter && companyBody) companyFooter.classList.toggle("elevated", companyBody.scrollTop > 0)
  }
  const handleEscCompany = (e) => {
  ...
  cancelCompanyBtn?.addEventListener("click", (e) => {
    e.preventDefault()
    closeCompany()
  })
  ...
  const updateVacFooterShadow = () => {
    if (vacFooter && vacBody) vacFooter.classList.toggle("elevated", vacBody.scrollTop > 0)
  }
  const handleEscVac = (e) => {
